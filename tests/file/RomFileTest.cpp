...
TEST(RomFile, Pc2SnesAdr)
{
	...
	LONGS_EQUAL(0x808000, target->Pc2SnesAdr(target, 0x0000));
	LONGS_EQUAL(0x80ffff, target->Pc2SnesAdr(target, 0x7fff));
}
...
TEST(RomFile, RatsSearch)
{
	...
	const uint32 RatsTag1Ptr = 0x808300;
	const uint8* RatsTag2 = (uint8*)"STAR\x00\x01\xff\xfe\x33\x45";
	const uint32 RatsTag2Ptr = 0x80a123;
	uint8* ptr;
	...
	LONGS_EQUAL(RatsTag2Ptr, target->RatsSearch(target, 0x9000, NULL));
	LONGS_EQUAL(ROMADDRESS_NULL, target->RatsSearch(target, 0xa200, NULL));
